...
export default function EditMedicineScreen() {
  ...
  const [currentDurationDays, setCurrentDurationDays] = useState(25);
  ...
  useEffect(() => {
    const loadMedicine = async () => {
      if (!medicineId) {
        Alert.alert('Missing medicine', 'Medicine details not found.');
        router.back();
        return;
      }
      ...
      try {
        ...
        const medicine = await medicineReminderService.getMedicineById(profile.id, medicineId);
        if (!medicine) {
          Alert.alert('Not found', 'Medicine not found.');
          router.back();
          return;
        }
        ...
        setSelectedSlots(medicine.scheduleSlots);
        setDurationDays(String(medicine.durationDays || 25));
        setCurrentDurationDays(medicine.durationDays || 25);
        setRemainingDays(getRemainingDays(medicine));
      } catch (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await medicineReminderService.updateMedicine(profile.id, medicineId, {
        name,
        dosage,
        notes,
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await medicineReminderService.deleteMedicine(profile.id, medicineId);
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Edit Medicine</Text>
        <Text style={styles.subtitle}>Update schedule and treatment duration</Text>
      </View>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <TouchableOpacity
          ...
          disabled={saving}
        >
          <Text style={styles.saveButtonText}>{saving ? 'Saving...' : 'Update Medicine'}</Text>
        </TouchableOpacity>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            <Text style={styles.modalMessage}>
              You have to consume this medicine for {currentDurationDays} days. {remainingDays} day
              {remainingDays === 1 ? '' : 's'} still left. Want to delete?
            </Text>

            <View style={styles.modalActions}>
              <TouchableOpacity
                style={[styles.modalButton, styles.modalDeleteYes, saving && styles.saveButtonDisabled]}
                onPress={handleDelete}
                disabled={saving}
              >
                <Text style={styles.modalDeleteYesText}>Yes</Text>
              </TouchableOpacity>

              <TouchableOpacity
                style={[styles.modalButton, styles.modalCancelNo]}
                onPress={() => setShowDeleteModal(false)}
                disabled={saving}
              >
                <Text style={styles.modalCancelNoText}>Cancel</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
